...
const LevelMicroPq4 = () => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        const res = await fetch("http://localhost/pidify/pidify/getFiles.php?level=400&type=pq_micro");
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, "notifications", "fpas", "items"), {
        message: `${selectedFiles.length} new microbiology past questions added 📚`,
        read: false,
  ...
  return (
   <div className="flex bg-gray-100 w-full overflow-x-hidden">
            ...
            <header className="flex flex-col md:flex-row md:items-center md:justify-between bg-white shadow-md px-4 md:px-8 py-4 w-full relative">
              ...
              <div className="flex items-center justify-between md:justify-end mt-3 md:mt-0 gap-4 md:gap-6 w-full md:w-auto">
                <div className="relative">
                  ...
                  <div className="text-2xl text-gray-600 hover:text-gray-800 cursor-pointer">➕</div>
                  <input type="file" multiple onChange={handleFileChange} className="hidden" />
                </label>                <div className="flex items-center gap-2 md:gap-3">
      ...
      <section className="flex-1 p-6 md:p-8 bg-gray-50 min-h-screen overflow-y-auto">
        <div className="mb-8">
          <h1 className="text-1.5xl md:text-1.5xl font-semibold text-gray-800 mb-4">Access 400 Level Micro-Biology Past Questions</h1>
          <input type="text" placeholder="Search for past questions..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full md:w-1/2 px-4 py-2 border border-gray-300 rounded-xl shadow-sm" />
        </div>
        <nav className="flex flex-wrap gap-3 mb-6">
          <Link to="/micro-biology-pq/100" className="px-4 py-2 bg-white border rounded-lg">100 LVL</Link>
          <Link to="/micro-biology-pq/200" className="px-4 py-2 bg-white border rounded-lg">200 LVL</Link>
          <Link to="/micro-biology-pq/300" className="px-4 py-2 bg-white border rounded-lg">300 LVL</Link>
          <Link to="/micro-biology-pq/400" className="px-4 py-2 bg-[#006666] text-white rounded-lg">400 LVL</Link>
        </nav>
