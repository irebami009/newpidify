...
const LevelBioChemPq3 = () => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        const res = await fetch("http://localhost/pidify/pidify/getFiles.php?level=300&type=pq_bio");
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, "notifications", "fpas", "items"), {
        message: `${selectedFiles.length} new biochemistry past questions added 📚`,
        read: false,
  ...
  return (
   <div className="flex bg-gray-100 w-full overflow-x-hidden">
      ...
      <section className="flex-1 p-6 md:p-8 bg-gray-50 min-h-screen overflow-y-auto">
        <div className="mb-8">
          <h1 className="text-1.5xl md:text-1.5xl font-semibold text-gray-800 mb-4">Access 300 Level Biochemistry Past Questions</h1>
        ...
        <nav className="flex flex-wrap gap-3 mb-6">
          <Link to="/bio-chemistry-pq/100" className="px-4 py-2 bg-white border rounded-lg">100 LVL</Link>
          <Link to="/bio-chemistry-pq/200" className="px-4 py-2 bg-white border rounded-lg">200 LVL</Link>
          <Link to="/bio-chemistry-pq/300" className="px-4 py-2 bg-[#006666] text-white rounded-lg">300 LVL</Link>
          <Link to="/bio-chemistry-pq/400" className="px-4 py-2 bg-white border rounded-lg">400 LVL</Link>
        </nav>
...
export default LevelBioChemPq3;
