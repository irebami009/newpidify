...
const LevelCompPq3 = () => {
  ...
  // HEADER STATES (same as LevelCompPq1)
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        const res = await fetch("http://localhost/pidify/pidify/getFiles.php?level=300&type=pq_comp");
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, "notifications", "fpas", "items"), {
        message: `${selectedFiles.length} new past question(s) added 📚`,
        read: false,
  ...
  return (
    <div className="flex bg-gray-100 w-full overflow-x-hidden">
      <div className="flex-1 flex flex-col w-full min-w-0">
        ...
        {/* NAVBAR (UNCHANGED DESIGN EXACTLY LIKE LEVELCOMP1) */}
        ...
        <section className="flex-1 p-6 md:p-8 bg-gray-50 min-h-screen overflow-y-auto">
          ...
          <div className="mb-8">
            <h1 className="text-1.5xl md:text-1.5xl font-semibold text-gray-800 mb-4">
              Access 300 Level Past Questions
            </h1>
          ...
          <nav className="flex flex-wrap gap-3 mb-6">
            <Link to="/computer-science/100level" className="px-4 py-2 bg-white text-gray-700 border rounded-lg">
              100 LVL
            </Link>

            <Link to="/computer-science/200level" className="px-4 py-2 bg-white text-gray-700 border rounded-lg">
              200 LVL
            </Link>

            <Link to="/computer-science/300level" className="px-4 py-2 bg-[#006666] text-white rounded-lg">
              300 LVL
            </Link>

            <Link to="/computer-science/400level" className="px-4 py-2 bg-white text-gray-700 border rounded-lg">
              400 LVL
            </Link>
          </nav>
          ...
          <main className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            {filteredFiles.map((file) => (
              <div
                key={file.id}
                className="bg-white rounded-2xl shadow-md p-4 text-center relative"
              >
                <button
                ...
                <a href={file.url} target="_blank" rel="noopener noreferrer">
                  <img
                    src={file.preview || "/Images/pdf-icon.png"}
                  ...
                  {file.pages && (
                    <p className="text-gray-500 text-sm">{file.pages} Pages</p>
                  )}
                </a>
              </div>
            ))}
