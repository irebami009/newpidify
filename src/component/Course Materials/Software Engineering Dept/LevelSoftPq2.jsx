...
const LevelSoftPq2 = () => {
  ...
  const markAllAsRead = async () => {
    await Promise.all(
      notifications.map((note) => {
        if (!note.read)
          return updateDoc(doc(db, "notifications", "fpas", "items", note.id), { read: true });
      })
    );
    setUnreadCount(0);
  };
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        const res = await fetch("http://localhost/pidify/pidify/getFiles.php?level=200&type=pq_soft");
        ...
        <section className="flex-1 p-6 md:p-8 bg-gray-50 min-h-screen overflow-y-auto">
          ...
          <div className="mb-8">
            <h1 className="text-1.5xl md:text-1.5xl font-semibold text-gray-800 mb-4">
              Access 200 Level Software Engineering Past Questions
            </h1>
          ...
          <nav className="flex flex-wrap gap-3 mb-6">
            <Link to="/software-engineering-pq/100" className="px-4 py-2 bg-white border rounded-lg">100 LVL</Link>
            <Link to="/software-engineering-pq/200" className="px-4 py-2 bg-[#006666] text-white rounded-lg">200 LVL</Link>
